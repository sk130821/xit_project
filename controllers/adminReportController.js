    ...
    try {
      ...
      const [invStats] = await conn.query(
        `SELECT
          ...
          SUM(plan_type = 'flexible_lock') as flex_lock_count,
          ...
          COALESCE(SUM(CASE WHEN plan_type = 'flexible_lock' THEN token_amount ELSE 0 END), 0) as flex_lock_amount,
      ...
      res.json({
        ...
        investments: {
          ...
          flex_lock_count: Number(inv.flex_lock_count),
          lock_amount: Number(inv.lock_amount),
          flex_amount: Number(inv.flex_amount),
          flex_lock_amount: Number(inv.flex_lock_amount),
